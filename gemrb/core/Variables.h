...
public:
	...
	template<typename STR, typename NUM, ENABLE_CHAR_RANGE(STR)>
	typename std::enable_if<std::is_integral<NUM>::value || std::is_enum<NUM>::value, bool>::type
	Lookup(const STR& key, NUM& rValue) const {
		ieDword val;
...
protected:
	...
	inline unsigned int MyCompareKey(const key_t&, key_t str) const;
