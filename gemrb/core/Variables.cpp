...
inline bool Variables::MyCopyKey(char*& dest, const key_t& key) const
{
	...
	//use j
	for (size_t i = 0; key[i] && j < key.length(); i++)
		if (key[i] != ' ') {
			j++;
		}
...
inline unsigned int Variables::MyCompareKey(const key_t& key, key_t str) const
{
	size_t i = 0,j = 0;
	for (; str[j] && key[i] && i < key.length() && j <str.length(); ++i, ++j) {
...
inline unsigned int Variables::MyHashKey(const key_t& key) const
{
	...
	for (int i = 0; i < key.length() && key[i]; i++) {
